...
public class FundTask {
    ...
    @XxlJob("fundJobHandler")
    public void fundJobHandler() {
        Date currentDate = new Date();
        XxlJobHelper.log("insertBatchHistory定时任务启动：当前时间为：" + DateUtil.format(DateUtil.yyyy_MM_dd_HH_mm, currentDate));
        int row = fundService.insertBatchHistory(DateUtil.subDate(currentDate, 1));
        XxlJobHelper.log("insertBatchHistory定时任务完成：当前时间为："
                + DateUtil.format(DateUtil.yyyy_MM_dd_HH_mm, new Date()) + ";row为：" + row);
    }
